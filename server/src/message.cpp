...
size_t MessageReader::expected_bytes_remaining() {
    if (m_bytes_read >= m_expected_size) return 0;
    else return m_expected_size - m_bytes_read;
}
...
std::expected<void, std::string> MessageReader::process_bytes(size_t n, const char* bytes) {
    ...
    if (m_expected_size == 0) {
        if (n < 4) return std::unexpected { "Invalid message header" };
        uint32_t size{};
        std::memcpy(&size, bytes, 4);

        if (size == 0 || size > message::constants::MAX_MESSAGE_SIZE) return std::unexpected { "Invalid message size" };
    ...
    if (num_bytes > expected_bytes_remaining()) return std::unexpected { "Cannot accept more bytes" };
